 ...
 */
class Modules_Services extends TOC_Controller
{
    /**
     * Constructor
     *
    ...
    public function install()
    {
        ...
        include_once '../system/tomatocart/libraries/service/service_' . $code . '.php';
        $class = config_item('subclass_prefix') . 'Service_'. $code;
        ...
        $data = array('title' => $class->get_title(), 'code' => $class->get_code(), 'author_name' => '', 'author_www' => '', 'modules_group' => 'service', 'params' => json_encode($config));
    ...
    public function uninstall()
    {
        ...
        if ($this->extensions_model->uninstall('service', $code)) {
            $response = array('success' => true, 'feedback' => lang('ms_success_action_performed'));
        } else {
            $response = array('success' => false, 'feedback' => lang('ms_error_action_not_performed'));
        }
    ...
    function save()
    {
        ...
        if ($this->extensions_model->save('service', $code, json_encode($configurations))) 
        {
            $response = array('success' => true, 'feedback' => lang('ms_success_action_performed'));
        } 
        else 
    ...
    function get_configuration_options()
    {
        ...
        //load language xml file
        $this->lang->xml_load('modules/service/' . $code);
        
        //include file
        require_once '../system/tomatocart/libraries/service/service_' . $code . '.php';
